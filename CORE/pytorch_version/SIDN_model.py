...
class Block(torch.nn.Module):
    ...
    def __init__(self, in_channel, out_channel, conv=None, batch_norm_momentum=0.01, last_decoder=False, acti = 'relu', layers=4):
        ...
        # selection matrix
        alpha = torch.nn.Parameter(torch.ones((self.layers + 1, self.layers - 1)), requires_grad=True)
    ...
    def forward(self, x, skip):
        ...
        if skip is not None:
            ...
            weights = torch.ones(self.layers - 1,).cuda(device) #cuda
    ...
    def __init__(self,
        ...
        for iteration in range(self.iterations):
            for layer in range(self.num_layers):
                ...
                # block = DecoderBlock(in_channel, out_channel, layers = self.num_layers)
